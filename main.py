...
def monitor_instances():
    while True:
        db = SessionLocal()
        try:
            running_instances = db.query(models.Instance).filter(
            ...
            for inst in running_instances:
                lifetime_minutes = (now - inst.created_at).total_seconds() / 60

                if inst.max_runtime and lifetime_minutes >= inst.max_runtime:
                    print(f"Инстанс {inst.id} превысил время жизни ({lifetime_minutes:.1f} мин)")

                    if inst.instance_type == "container":
                        docker_manager.stop_container(inst.container_id)
                        inst.status = "expired"
                    elif inst.instance_type == "vm":
                        qemu_manager.stop_vm(inst.pid)
                        inst.status = "expired"

                    db.commit()
                    continue

                if inst.instance_type == "container" and inst.container_id:
                    try:
                        container = docker_manager.client.containers.get(inst.container_id)
                        if container.status != "running":
                            print(f"Контейнер {inst.container_id} имеет статус {container.status}")
                            inst.status = container.status
                            db.commit()
                    except:
                        print(f"Контейнер {inst.container_id} не найден в Docker, удаляем из БД")
                        db.delete(inst)
                        db.commit()
...
async def create_instance(instance: schemas.InstanceCreate, db: Session = Depends(get_db)):
    ...
    if instance.type == "container":
        print(f"Пытаюсь создать контейнер с параметрами: {instance}")
        try:
            ...
            status = "running"
            print(f"Контейнер создан: ID={container_id}, порт={ssh_port}")
        except Exception as e:
            print(f"ОШИБКА В create_container: {e}")
            import traceback
            traceback.print_exc()
            raise HTTPException(status_code=500, detail=f"Failed to create container: {str(e)}")

    elif instance.type == "vm":
        print(f"Пытаюсь создать VM с параметрами: {instance}")
        try:
            ...
            status = "running"
            print(f"VM создана: PID={pid}, порт={ssh_port}")
        except Exception as e:
            print(f"ОШИБКА В create_vm: {e}")
            import traceback
            traceback.print_exc()
    ...
    print(f"Сохраняю в БД: type={instance.type}, status={status}")
    ...
    print(f"Запись в БД создана с ID={db_instance.id}")
...
async def stop_instance(instance_id: int, db: Session = Depends(get_db)):
    ...
    if db_instance.instance_type == "container" and db_instance.container_id:
        ...
        if success:
            ...
            db.commit()
            return {"message": "Container stopped (will be auto-removed)"}
        else:
